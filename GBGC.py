...
def init_GB_graph(graph, init_GB_num):
    """
    Initialize the GB graph by selecting the center nodes and creating initial subgraphs (GBs).
    
    Args:
        graph (networkx.Graph): The input graph.
        init_GB_num (int): The number of initial GBs to create.

    Returns:
        list: A list of initial GB subgraphs.
    """
...
def Calculate_purity(graph):
    """
    Calculate the purity of the graph based on node labels.
    
    Args:
        graph (networkx.Graph): The input graph.

    Returns:
        float: The purity of the graph.
    """
...
def Calculate_qity_purity(graph):
    """
    Calculate a combined metric of average degree and purity.
...
def split_ball_split(graph):
    """
    Split the graph into two clusters based on the highest degree nodes.
    
    Args:
        graph (networkx.Graph): The input graph.

    Returns:
        tuple: Two subgraphs resulting from the split.
    """
    degree_dict = dict(graph.degree())
    sorted_nodes = sorted(degree_dict, key=degree_dict.get, reverse=True)
...
def split_ball(graph, split_GB_list, purity):
    """
    Recursively split the graph into subgraphs based on a purity criterion.
    
    Args:
        graph (networkx.Graph): The input graph.
        split_GB_list (list): List to store resulting subgraphs.
        purity (int or float): Purity criterion for splitting.
    """
    if len(graph) == 1:
...
def get_node_tag(graph):
    """
    Get the most common label in the graph.
    
    Args:
        graph (networkx.Graph): The input graph.

    Returns:
        int or str: The most common label.
    """
    node_labels = list(nx.get_node_attributes(graph, 'label').values())
...
def get_GB_graph(graph, purity, degree_purity):
    """
    Create a GB graph by coarsening the input graph based on the specified purity criterion.
    
    Args:
        graph (networkx.Graph): The input graph.
        purity (int or float): Purity criterion for coarsening.
        degree_purity (bool): Whether to use degree-based purity first.

    Returns:
        tuple: The GB graph and corresponding node tags.
    """
    if len(graph) <= 2:
...
def add_GB_aft_attr(graph):
    """
    Add GB attributes to the graph based on various graph metrics.
    
    Args:
        graph (networkx.Graph): The input graph.

    Returns:
        torch.Tensor: The GB attributes.
    """
    ...
    eccentricity = sum(dict(nx.eccentricity(graph)).values()) / graph.number_of_nodes()
    try:
        eigenvector_centrality = sum(dict(nx.eigenvector_centrality(graph)).values()) / num_nodes
    except:
        eigenvector_centrality = 0

    GB_aft_attr = torch.tensor([num_nodes, num_edges, edge_betweenness_centrality, closeness_centrality, 
    eigenvalues, density, diameter, average_cc, eccentricity, eigenvector_centrality])
    return GB_aft_attr

def GB_coarsen(graph, node_tags, purity, degree_purity):
    """
    Coarsen the input graph based on the specified purity criterion and node tags.
    
    Args:
        graph (networkx.Graph): The input graph.
        node_tags (list): The node tags.
        purity (int or float): Purity criterion for coarsening.
        degree_purity (bool): Whether to use degree-based purity first.

    Returns:
        tuple: The coarsened GB graph and corresponding node tags.
    """
